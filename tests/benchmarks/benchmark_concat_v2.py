...
if __name__ == "__main__":
    ...
    time_join_gen = timeit.timeit(lambda: concat_join_gen(segments), number=num_iterations)

    print(f"Loop concatenation: {time_loop:.4f} seconds")
    print(f"Join (list comp):   {time_join_list:.4f} seconds")
    print(f"Join (gen expr):    {time_join_gen:.4f} seconds")
